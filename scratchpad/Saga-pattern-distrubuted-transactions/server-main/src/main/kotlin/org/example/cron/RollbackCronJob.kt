...
class RollbackCronJob(
    ...
    fun initiateRollbackActions() {
        ...
        actions.forEach { request ->
            ...
            log.info(
                "Needs to rollback! [globalId={};actions={}]",
                request.id,
            ...
            request.pendingActions.keys.forEach {
                ...
                when (it) {
                    PendingActionName.CREATE_ORDER -> orderService.stopProcessingBlocking(action)
                    PendingActionName.PROCESS_PAYMENT -> paymentService.stopProcessingBlocking(action)
                    PendingActionName.RESERVE_ITEM -> itemService.stopProcessingBlocking(action)
                    PendingActionName.UNKNOWN -> {
                        log.warn("Unknown action $action")
                        counter-- // cancels out the counter++
                    }
                }
                processingService.handleResponse(action.rollback())

                counter++
            }
        }

        if (counter > 0) log.info("cleaning up old actions [rollbackCountInitiated=$counter]")
    }
